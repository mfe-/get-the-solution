...
Magnitude is the absolute value of any value, as opposed to its phase. absolute value of H(e^{jω}) 
...
Get the discrete time signal with:

$Xs(t)=X(t)*S(t)=\sum_{\infty}^{n=-\infty}  x[n]\cdot \delta(t-nTs)$
