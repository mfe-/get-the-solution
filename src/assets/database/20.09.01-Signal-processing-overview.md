...
continuous periodic signal could be represented as the sum of properly chosen sinusoidal waves. As a signal can be continuous or discrete and signals can be periodic or aperiodic will result in up to four categories.
...
Transforming from the frequency domain to the time domain is called synthesis, or the inverse DFT.
