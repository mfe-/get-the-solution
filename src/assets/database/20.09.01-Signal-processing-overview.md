...
There are some common representations for the transformed signal which are often called Amplitude, Magnitude or  Specturm. The spectrum shows all frequencies which the signal contains (Can be written as $\sphericalangle X(e^{j\omega})$). The Magnitude is the absolute value of any value, as opposed to its phase. For example the absolute value of $|H(e^{j\omega})|$.
...
### Properties of Discret Fourier Transform

#### Parseval's theorem

The energy in both Domains (time and frequency) is the same

$\sum_{\infty}^{n=-\infty} | x[n] |^2=\frac{1}{2\cdot\pi}\int_{-\pi}^{\pi} |X(\omega)|^2 d\omega$
...
convolution in time domain is equaivalent to a multiplication in frequency domain.  
multiplication in the time domain it will be a convolution in the frequeuncy domain. mulitplication in one domain is convolution in the other domain and visa verse.

$DTFT\{h[n]\cdot x[n]\}=H(\omega)* X(\omega)$

#### Symmetries in the DTFT
...
for real valued signals the amplitudes of the negative frequencies are the same as the psoitive frequencies and the phases are symmetric. that means the positive frequencies contains all information about the spectrum.

Continue to [Sampling and filtering](blog/Signal%20sampling%20and%20filtering)
