...
A more password less approach could eliminate the need of managing those credentials.

With Azure App Registration and Azure Managed Identity its possible to "run" your app with an Azure Active Directory user which is in this constellation a so called "service principal". The service principal will be granted to get access for example to Blob Storage, App Configuration and so on. The result of this set up requires the app only to maintain the credentials to identify it as the "registered app" which correspond to the service principal.
...
To access any Azure Resources (like the Azure App Configuration) we send the credentials of our registered app to the azure authentication resource endpoint (for example App Configuration, GraphApi, Blob Storage ...) to acquire an a access token for the requested Azure resource. The azure resource endpoint will be identified by a so called scope. You can recognize it later from the following pattern `api:\appresource\.default`. Whenever we interact with the azure resource we need to send the acquired token with the header of the request.
